...
# generate table 17.5 page 613 of [1]
...
savefile(l, '/tmp/as-17-elliptic-integrals-table17.5a.txt')
...
l += '\n'
l += gentable(phis, np.linspace(5,85,9))
savefile(l, '/tmp/as-17-elliptic-integrals-table17.5b.txt')
...
l += '\n'
l += gentable(phis, np.linspace(5,85,9))
savefile(l, '/tmp/as-17-elliptic-integrals-table17.5c.txt')

# data for comparison
phis = [0, 30, 45, 60, 90]
alps = [5, 20, 50, 60, 90]
l = gendata(phis, alps)
savefile(l, '/tmp/as-17-elliptic-integrals-table17.5-small.cmp')

# data for comparison
phis = np.linspace(0,90,19)
alps = np.sort(np.hstack((np.linspace(0,90,46), np.linspace(5,85,9))))
l = gendata(phis, alps)
savefile(l, '/tmp/as-17-elliptic-integrals-table17.5-big.cmp')
