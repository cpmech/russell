...
/// dx │x=at_x
/// ```
pub fn deriv1_forward7<F, A>(at_x: f64, args: &mut A, mut f: F) -> Result<f64, StrError>
where
...
#[cfg(test)]
mod tests {
    use super::deriv1_forward7;
    use crate::check::approx_eq;
    use crate::check::testing;

    #[test]
    fn deriv1_forward7_works() {
        ...
        for test in &tests {
            let args = &mut 0;
            let num = deriv1_forward7(test.at_x, args, test.f).unwrap();
