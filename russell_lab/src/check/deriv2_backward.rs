...
/// Stepsize h for deriv2_backward9
const STEPSIZE_BACKWARD9: f64 = 1e-3;
...
/// Approximates the second derivative using backward difference with 9 points
///
...
/// dx² │x=at_x
/// ```
pub fn deriv2_backward9<F, A>(at_x: f64, args: &mut A, mut f: F) -> Result<f64, StrError>
where
    F: FnMut(f64, &mut A) -> Result<f64, StrError>,
{
    let h = STEPSIZE_BACKWARD9;
    let hh = h * h;
    let f0 = f(at_x, args)?;
...
#[cfg(test)]
mod tests {
    use super::deriv2_backward9;
    use crate::check::approx_eq;
    use crate::check::testing;

    #[test]
    fn deriv2_backward9_works() {
        ...
        for test in &tests {
            let args = &mut 0;
            let num = deriv2_backward9(test.at_x, args, test.f).unwrap();
