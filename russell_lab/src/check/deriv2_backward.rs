...
/// Stepsize h for deriv2_backward8
const STEPSIZE_BACKWARD8: f64 = 1e-3;
...
/// dx² │x=at_x
/// ```
pub fn deriv2_backward8<F, A>(at_x: f64, args: &mut A, mut f: F) -> Result<f64, StrError>
where
    F: FnMut(f64, &mut A) -> Result<f64, StrError>,
{
    let h = STEPSIZE_BACKWARD8;
    let hh = h * h;
    let f0 = f(at_x, args)?;
...
mod tests {
    ...
    fn deriv2_backward9_works() {
        ...
        for test in &tests {
            let args = &mut 0;
            let num = deriv2_backward8(test.at_x, args, test.f).unwrap();
