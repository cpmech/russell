...
mod tests {
    ...
    fn bessel_jn_with_n0_n1_works() {
        ...
        let mathematica = [
            ...
            (1,  0.0, 1e-50,  0.0),
            (1,  5.0, 1e-50, -0.32757913759146522203773432191016913276084990462405),
            (1, 10.0, 1e-16,  0.043472746168861436669748768025859288306272867118599),
        ];
    ...
    fn bessel_jn_with_negative_n_works() {
        ...
        let mathematica = [
            ...
            (-4, 18.0, 1e-16,  0.069639512651394864261741057764135700037439539390376),
            ...
            (-2, 18.0, 1e-17, -0.0075325148878013995602951948173734514372299122116846),
        ];
        for (n, x, tol, reference) in mathematica {
            // println!("n = {}, x = {:?}", n, x);
            approx_eq(bessel_jn(n, x), reference, tol);
    ...
    fn bessel_jn_edge_cases_work() {
        ...
        for n in 2..=7 {
            // println!("n = {}", n);
            let val = bessel_jn(n, TWO_302);
            assert!(f64::abs(val) > 0.0);
            assert!(f64::abs(val) < 1e-45);
        }
