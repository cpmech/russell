...
mod tests {
    ...
    fn special_values_works() {
        ...
        for n in 0..10 {
            let expected = if n % 2 == 0 { (n + 1) as f64 } else { -((n + 1) as f64) };
            approx_eq(chebyshev_un(n, -1.0), expected, 1e-15);
        }
    ...
    fn derivatives_exact_formula_works() {
        ...
        for x in points {
            ...
            // n=2
            approx_eq(chebyshev_un_deriv1(2, x), 8.0 * x, 1e-13);
    ...
    fn deriv2_differential_equation_works() {
        ...
        for x in points {
            for n in 0..=5 {
                ...
                let residual = (1.0 - x * x) * u_n_double_prime - 3.0 * x * u_n_prime + n_f64 * (n_f64 + 2.0) * u_n;
