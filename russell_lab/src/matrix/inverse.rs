...
pub fn inverse(ai: &mut Matrix, a: &Matrix) -> Result<f64, &'static str> {
    ...
    if m == 2 {
        ...
        ai.data[0 + 0 * m] = a.data[1 + 1 * m] / det;
        ai.data[0 + 1 * m] = -a.data[0 + 1 * m] / det;
        ai.data[1 + 0 * m] = -a.data[1 + 0 * m] / det;
        ai.data[1 + 1 * m] = a.data[0 + 0 * m] / det;
        return Ok(det);
    }
    ...
    // copy a into ai
    let m_i32 = to_i32(m);
    dcopy(m_i32 * m_i32, &a.data, 1, &mut ai.data, 1);

    // handle large matrix
    let lda_i32 = m_i32;
    let mut ipiv = vec![0_i32; m];
    dgetrf(m_i32, m_i32, &mut ai.data, lda_i32, &mut ipiv)?;
    ...
    for i in 0..m_i32 {
        ...
        if ipiv[iu] - 1 == i {
            det = det * ai.data[iu + iu * m];
        } else {
            det = -det * ai.data[iu + iu * m];
        }
    }
    // second, perform the inversion
    dgetri(m_i32, &mut ai.data, lda_i32, &ipiv)?;
