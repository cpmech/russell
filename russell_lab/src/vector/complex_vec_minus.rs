...
/// }
/// ```
pub fn complex_vec_minus(w: &mut ComplexVector, u: &ComplexVector, v: &ComplexVector) -> Result<(), StrError> {
    array_minus_op_complex(w.as_mut_data(), u.as_data(), v.as_data())
}
...
#[cfg(test)]
mod tests {
    use super::complex_vec_minus;
    use crate::{complex_vec_approx_eq, cpx, Complex64, ComplexVector};

    #[test]
    fn complex_vec_minus_fail_on_wrong_dims() {
        ...
        assert_eq!(complex_vec_minus(&mut w_2, &u_3, &v_2), Err("arrays are incompatible"));
        assert_eq!(complex_vec_minus(&mut w_2, &u_2, &v_3), Err("arrays are incompatible"));
    }

    #[test]
    fn complex_vec_minus_works() {
        ...
        complex_vec_minus(&mut w, &u, &v).unwrap();
        #[rustfmt::skip]
        let correct = &[
            cpx!(0.5,0.0), cpx!(1.0,0.0),
    ...
    #[test]
    fn complex_vec_minus_sizes_works() {
        const NOISE: Complex64 = cpx!(1234.567, 3456.789);
        for size in 0..13 {
