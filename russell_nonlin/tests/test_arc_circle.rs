...
fn test_arc_circle_2() {
    ...
    approx_eq(state.u[0], RADIUS, 1e-9);
    ...
    if SAVE_FIGURE {
        ...
        let duds = out.get_duds_values(0);
        let dlds = out.get_dlds_values();
        do_plot("test_arc_circle_2", uu, ll, duds, dlds, hh);
        do_plot_stepsizes("test_arc_circle_2", hh);
    }
}
