...
fn test_reaction_diffusion_1d() {
    ...
    let mut fdm = FdmLaplacian1d::new(1.0, 0.0, 1.0, NDIV, None).unwrap();
    ...
    let norm_type = Norm::Max;
    let status = solver
        .solve(
            &mut 0,
            &mut state,
            IniDir::Pos,
            // Stop::MaxNormU(200.0, Norm::Max),
            Stop::MaxLambda(3.51),
            AutoStep::Yes,
            Some(out),
        )
    ...
    if SAVE_FIGURE {
        ...
        let y_lbl = if norm_type == Norm::Max {
            "‖u‖∞"
        } else {
            "‖u‖₂"
        };
        plot.add(&curve)
            .grid_and_labels("λ", y_lbl)
            .save(&format!("/tmp/russell_nonlin/test_reaction_diffusion_1d.svg"))
            .unwrap();
    }
}
