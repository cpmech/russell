...
#[test]
fn test_1d_prob03_spc_sps() -> Result<(), StrError> {
    // problem setup
    ...
    spc.for_each_coord(|m, x| {
        ...
        approx_eq(a[m], analytical(x), 4.1e-7);
    });
    ...
    if SAVE_FIGURE {
        ...
        plot.add(&curve_ana)
            ...
            .save("/tmp/russell_pde/test_1d_prob03_spc_sps.svg")?;
    }
    Ok(())
}

#[test]
fn test_1d_prob03_spc_lmm() -> Result<(), StrError> {
    // problem setup
    let (xmin, xmax, kx, alpha, ebcs, nbcs, source, analytical) = ProblemSamples::d1_problem_03();
