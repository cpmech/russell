...
#[test]
fn test_1d_prob04a_spc_sps() -> Result<(), StrError> {
    let (nn, tol) = (5, 3.463e-2);
    // let (nn, tol) = (16, 1.261e-10); // Trefethen, page 64, Output 13

    // problem setup
    let (xmin, xmax, kx, ebcs, nbcs, source, analytical) = ProblemSamples::d1_problem_04a();
    ...
    if SAVE_FIGURE {
        ...
        plot.add(&curve_ana)
            ...
            .save("/tmp/russell_pde/test_1d_prob04a_spc.svg")?;
    }
    Ok(())
}

#[test]
fn test_1d_prob04b_spc_sps() -> Result<(), StrError> {
    ...
    if SAVE_FIGURE {
        ...
        plot.add(&curve_ana)
            .add(&curve_num)
            .grid_labels_legend("$x$", "$\\phi$")
            .save("/tmp/russell_pde/test_1d_prob04b_spc.svg")?;
    }
    Ok(())
}
