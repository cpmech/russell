...
fn test_2d_prob05_spc() -> Result<(), StrError> {
    ...
    // plot results
    if SAVE_FIGURE {
        let fn_a = "/tmp/russell_pde/test_2d_prob05_spc_a.svg".to_string();
        let fn_b = "/tmp/russell_pde/test_2d_prob05_spc_b.svg".to_string();
...
fn test_2d_prob05_spc_map() -> Result<(), StrError> {
    ...
    // get the problem data
    let (xmin, xmax, ymin, ymax, k, _, ebcs, nbcs, source, analytical) = ProblemSamples::d2_problem_05();
