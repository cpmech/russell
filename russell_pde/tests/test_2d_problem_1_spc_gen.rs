...
use russell_pde::{EssentialBcs2d, Grid2d, NaturalBcs2d, SpcMap2d, StrError, TransfiniteSamples};
...
// ∇²ϕ = -1
//
// on a transfinite mapped domain. The mapped domain is a [-1,1]×[-1,1] square.
// The boundary conditions are:
//
// R-min (aka Xmin): ∇ϕ = 0   on r = -1
// R-max (aka Xmax):  ϕ = 0   on r =  1
// S-min (aka Ymin): ∇ϕ = 0   on s = -1
...
fn run_test(nn: usize, tol: f64) -> Result<f64, StrError> {
    ...
    let mut spc = SpcMap2d::new(grid, ebcs, nbcs, map)?;
