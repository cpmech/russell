...
struct InterfaceComplexCUDSS {
    ...
    cuDoubleComplex *gpu_values;

    /// @brief GPU (device) right-hand side
    cuDoubleComplex *gpu_b;

    /// @brief GPU (device) solution vector
    cuDoubleComplex *gpu_x;
...
extern "C" struct InterfaceComplexCUDSS *complex_solver_cudss_new() {
    ...
    struct InterfaceComplexCUDSS *solver = (struct InterfaceComplexCUDSS *)calloc(1, sizeof(struct InterfaceComplexCUDSS));
    if (solver == NULL) {
...
extern "C" void complex_solver_cudss_drop(struct InterfaceComplexCUDSS *solver) {
    if (solver == NULL) {
        return;
    }
...
extern "C" int32_t complex_solver_cudss_initialize(struct InterfaceComplexCUDSS *solver,
                                                   int32_t ordering,
                                                   int32_t matching,
                                                   int32_t pivoting,
    ...
    /* Show message */
    if (verbose == C_TRUE) {
        printf("complex_solver_cudss_initialize: Memory allocated\n");
    }
    ...
    cuda_error = cudaMemcpy(solver->gpu_values, values, nnz * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
    if (cuda_error != cudaSuccess) {
        return ERROR_CUDA_MEMCPY;
    }

    /* Create object for the right-hand side b (as dense matrix) */
    cudssStatus_t status = cudssMatrixCreateDn(&solver->b_vec, ndim, 1, ndim, solver->gpu_b, CUDSS_C_64F, CUDSS_LAYOUT_COL_MAJOR);
    if (status != CUDSS_STATUS_SUCCESS) {
        return ERROR_CUDSS_MATRIX_CREATE_DN;
    }

    /* Create object for the solution x (as dense matrix) */
    status = cudssMatrixCreateDn(&solver->x_vec, ndim, 1, ndim, solver->gpu_x, CUDSS_C_64F, CUDSS_LAYOUT_COL_MAJOR);
    if (status != CUDSS_STATUS_SUCCESS) {
        return ERROR_CUDSS_MATRIX_CREATE_DN;
    }
    ...
    status = cudssMatrixCreateCsr(&solver->aa_mat, ndim, ndim, nnz, solver->gpu_row_pointers, NULL,
                                  ...
                                  CUDSS_R_32I, CUDSS_C_64F, mtype, mview,
                                  CUDSS_BASE_ZERO);
    if (status != CUDSS_STATUS_SUCCESS) {
    ...
    /* Show message */
    if (verbose == C_TRUE) {
        printf("complex_solver_cudss_initialize: Symbolic factorization completed\n");
    }
...
extern "C" int32_t complex_solver_cudss_factorize(struct InterfaceComplexCUDSS *solver,
    ...
    if (verbose == C_TRUE) {
        ...
        if (npivots > 0) {
            printf("complex_solver_cudss_factorize: WARNING: %d pivot(s) perturbed (matrix may be (nearly) singular)\n", npivots);
        }
    }
    ...
    *effective_pivoting = used_pivot;
    ...
    /* Show message */
    if (verbose == C_TRUE) {
        printf("complex_solver_cudss_factorize: Numeric factorization completed\n");
    }
...
extern "C" int32_t complex_solver_cudss_solve(struct InterfaceComplexCUDSS *solver,
                                              cuDoubleComplex *x,
    ...
    cudaError_t cuda_error = cudaMemcpy(solver->gpu_b, rhs, ndim * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
    if (cuda_error != cudaSuccess) {
        return ERROR_CUDA_MEMCPY;
    }
    ...
    cuda_error = cudaMemcpy(x, solver->gpu_x, ndim * sizeof(cuDoubleComplex),
    ...
    /* Show message */
    if (verbose == C_TRUE) {
        printf("complex_solver_cudss_solve: Solution completed\n");
    }
