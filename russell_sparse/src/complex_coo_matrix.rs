...
mod tests {
    ...
    fn assign_capture_errors() {
        ...
        let mut a_1x2 = ComplexCooMatrix::new(1, 2, nnz_a, None, false).unwrap();
        let b_2x1 = CooMatrix::new(2, 1, nnz_b, None, false).unwrap();
        let b_1x3 = CooMatrix::new(1, 3, nnz_b, None, false).unwrap();
        let b_1x2_sym = CooMatrix::new(1, 2, nnz_b, sym, false).unwrap();
        let b_1x2_one = CooMatrix::new(1, 2, nnz_b, None, true).unwrap();
        let mut b_1x2 = CooMatrix::new(1, 2, nnz_b, None, false).unwrap();
        ...
        assert_eq!(
            a_1x2.assign_real(2.0, 3.0, &b_1x2_one).err(),
    ...
    fn assign_works() {
        let nnz = 2;
        let mut a = ComplexCooMatrix::new(3, 2, nnz, None, false).unwrap();
        let mut b = CooMatrix::new(3, 2, nnz, None, false).unwrap();
    ...
    fn augment_capture_errors() {
        ...
        let nnz_a = 1;
        let nnz_b = 1;
        let mut a_1x2 = ComplexCooMatrix::new(1, 2, nnz_a /* + nnz_b */, None, false).unwrap();
        let b_2x1 = CooMatrix::new(2, 1, nnz_b, None, false).unwrap();
        let b_1x3 = CooMatrix::new(1, 3, nnz_b, None, false).unwrap();
        let b_1x2_sym = CooMatrix::new(1, 2, nnz_b, sym, false).unwrap();
        let b_1x2_one = CooMatrix::new(1, 2, nnz_b, None, true).unwrap();
        ...
        assert_eq!(
            a_1x2.augment_real(2.0, 3.0, &b_1x2_one).err(),
            Some("matrices must have the same one_based")
        );
        assert_eq!(
    ...
    fn augment_works() {
        let nnz_a = 1;
        let nnz_b = 2;
        let mut a = ComplexCooMatrix::new(3, 2, nnz_a + nnz_b, None, false).unwrap();
        let mut b = CooMatrix::new(3, 2, nnz_b, None, false).unwrap();
        a.put(2, 1, cpx!(1000.0, 2000.0)).unwrap();
        b.put(0, 0, 10.0).unwrap();
        b.put(2, 1, 20.0).unwrap();
        assert_eq!(
