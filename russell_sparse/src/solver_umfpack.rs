...
impl LinSolTrait for SolverUMFPACK {
    /// Performs the solver setup (for UMFPACK: factorization, analysis, and initialization if needed)
    ///
    /// # Input
    ///
    /// * `mat` -- the coefficient matrix A, wrapped in a unified `SparseMatrix` enum.
    ///   **UMFPACK requires the inner matrix to be in COO format** (use `SparseMatrix::Coo(...)`).
