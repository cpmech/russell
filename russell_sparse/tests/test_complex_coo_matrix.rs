...
fn test_complex_coo_matrix() -> Result<(), StrError> {
    ...
    let mut coo = ComplexCooMatrix::new(3, 3, 4, sym, false)?;
    ...
    complex_vec_approx_eq(v.as_data(), &[cpx!(6.6, 4.7), cpx!(15.4, 7.6), cpx!(7.2, 8.8)], 1e-15);

    coo.mat_vec_mul(&mut v, cpx!(1.0, 2.0), &u)?;
    complex_vec_approx_eq(
        v.as_data(),
        &[cpx!(-2.8, 17.9), cpx!(0.2, 38.4), cpx!(-10.4, 23.2)],
        1e-14,
    );
    Ok(())
}
