...
///
/// A panic will occur if `sigma` is not symmetric, i.e., N = 9.
pub fn deriv1_invariant_sigma_t<const N: usize>(d1: &mut Tensor2<N>, sigma: &Tensor2<N>) -> Option<f64> {
    assert!(N != 9, "the stress tensor must be symmetric with N = 4 or N = 6");
    let jj2 = sigma.invariant_jj2();
    if jj2 > TOL_J2 {
...
///
/// A panic will occur if `sigma` is not symmetric, i.e., N = 9.
pub fn deriv1_invariant_q<const N: usize>(d1: &mut Tensor2<N>, sigma: &Tensor2<N>) -> Option<f64> {
    assert!(N != 9, "the stress tensor must be symmetric with N = 4 or N = 6");
    let jj2 = sigma.invariant_jj2();
    if jj2 > TOL_J2 {
...
mod tests {
    ...
    fn analytical_deriv<const N: usize>(fn_name: F, d1: &mut Tensor2<N>, sigma: &Tensor2<N>) {
        match fn_name {
            F::Norm => {
    ...
    struct ArgsNumDeriv<const N: usize> {
        ...
        sigma: Tensor2<N>, // temporary tensor with varying ij-components
    ...
    struct ArgsNumDerivM<const N: usize> {
        fn_name: F,        // name of f(σ)
        sigma: Tensor2<N>, // @ σ, with varying m-components
        m: usize,          // index m of ∂f/∂σₘ
    }

    // computes f(σ) for varying components x = σᵢⱼ
    fn f_sigma<const N: usize>(x: f64, args: &mut ArgsNumDeriv<N>) -> Result<f64, StrError> {
    ...
    fn f_sigma_mat<const N: usize>(x: f64, args: &mut ArgsNumDerivM<N>) -> Result<f64, StrError> {
    ...
    fn numerical_deriv<const N: usize>(sigma: &Tensor2<N>, fn_name: F) -> Matrix {
    ...
    fn numerical_deriv_mat<const N: usize>(sigma: &Tensor2<N>, fn_name: F) -> Matrix {
    ...
    fn check_deriv<const N: usize>(fn_name: F, sample: &SampleTensor2, tol: f64, _verbose: bool) {
        let sigma = Tensor2::<N>::from_std_matrix(&sample.matrix).unwrap();
