...
impl Tensor2 {
    ...
    pub fn deviator_determinant(&self) -> f64 {
        ...
        match self.vec.dim() {
            4 => {
                ((a[0] + a[1] - 2.0 * a[2])
                    * (4.0 * a[0] * a[0] + 4.0 * a[1] * a[1] + 2.0 * a[1] * a[2] - 2.0 * a[2] * a[2]
                        + 2.0 * a[0] * (-5.0 * a[1] + a[2])
                        + 9.0 * a[3] * a[3]))
                    / 54.0
            }
            6 => {
                (4.0 * a[0] * a[0] * a[0] + 4.0 * a[1] * a[1] * a[1] - 6.0 * a[1] * a[1] * a[2]
                    + 4.0 * a[2] * a[2] * a[2]
                    - 6.0 * a[0] * a[0] * (a[1] + a[2])
                    - 18.0 * a[2] * a[3] * a[3]
                    + 9.0 * a[2] * a[4] * a[4]
                    + 27.0 * SQRT_2 * a[3] * a[4] * a[5]
                    + 9.0 * a[2] * a[5] * a[5]
                    + a[1] * (-6.0 * a[2] * a[2] + 9.0 * (a[3] * a[3] + a[4] * a[4] - 2.0 * a[5] * a[5]))
                    - 3.0
                        * a[0]
                        * (2.0 * a[1] * a[1] - 8.0 * a[1] * a[2] + 2.0 * a[2] * a[2]
                            - 3.0 * (a[3] * a[3] - 2.0 * a[4] * a[4] + a[5] * a[5])))
                    / 54.0
            }
            _ => {
                ((2.0
                    ...
                    * (4.0 * a[0] * a[0] + 4.0 * a[1] * a[1] + 2.0 * a[1] * a[2] - 2.0 * a[2] * a[2]
                    ...
                    + SQRT_2
                        * (a[5] - a[8])
                        * ((a[3] + a[6]) * (a[4] + a[7]) + (SQRT_2 * (a[0] - 2.0 * a[1] + a[2]) * (a[5] + a[8])) / 3.0)
                    - SQRT_2
                        * (a[4] - a[7])
                        * ((SQRT_2 * (2.0 * a[0] - a[1] - a[2]) * (a[4] + a[7])) / 3.0 - (a[3] - a[6]) * (a[5] + a[8])))
                    / 4.0
            }
        }
    }
